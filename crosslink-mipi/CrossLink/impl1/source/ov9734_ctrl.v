...
module ov9734_ctrl (
    ...
    output reg [3:0]  rd_byte_num,   // number of bytes per read: 2 for ID, 1 for 4A00
    output reg      cfg_done,
    ...
    localparam NUM_REGS     = 176;
    localparam WAIT_NORMAL  = 16'd9_999;        // 200us between writes @ 50MHz
    localparam WAIT_SWRST   = 17'd99_999;       // 2ms after SW reset (0x0103)
    localparam WAIT_RD_LOOP = 23'd4_999_999;    // 100ms between 0x4A00 reads @ 50MHz

    // ── flag_done posedge detector ──
    ...
    // ── Register LUT: combinational (not clocked) — no wasted flip-flops ──
    reg [23:0] reg_cfg;
    reg [7:0]  reg_index;
    always @(*) begin
        case (reg_index)
            ...
            // ── Resize: 1288x728 → 1280x720 ──
            165: reg_cfg = {16'h0100, 8'h00}; // standby
            ...
            174: reg_cfg = {16'h5080, 8'h80}; // color bar test

            // ── Stream ON ──
            175: reg_cfg = {16'h0100, 8'h01};
    ...
    localparam S_RD_ID     = 3'd4;   // read Sensor ID: 0x300A (16-bit)
    localparam S_RD_4A00   = 3'd5;   // wait 100ms + read 0x4A00 (8-bit) polling loop

    reg [2:0]  state;
    reg [22:0] wait_cnt;         // widened for WAIT_RD_LOOP (23 bits)
    reg        delay_swrst;
    reg [19:0] final_cnt;
    reg [1:0]  rd_cnt;           // S_RD_ID debounce counter
        ...
        if (!sys_rst_n) begin
            ...
            case (state)
                ...
                S_RD_ID: begin
                    ...
                    else if (rd_cnt < 2'd3) begin
                        rd_cnt <= rd_cnt + 1'd1;        // wait for residual posedge to clear
                    end
                    else begin
                        start <= 1'b1;                   // unconditional start
                    end
                end
                ...
                S_RD_4A00: begin
                    ...
                    if (flag_done_posedge && start) begin
                        ...
                        start          <= 1'b0;
                        reg_4a00_val   <= data_read;
                        reg_4a00_valid <= 1'b1;
                        wait_cnt       <= 23'd0;
                    end
                    else if (!start) begin
                        // Idle: wait 100ms before next read
                        reg_4a00_valid <= 1'b0;
                        if (wait_cnt == WAIT_RD_LOOP) begin
                            wait_cnt <= 23'd0;
                            start    <= 1'b1;             // initiate 8-bit read
                        end
                        else begin
                            wait_cnt <= wait_cnt + 1'd1;
                        end
                    end
                    // else: start=1, read in progress — hold and wait for flag_done_posedge
                end
